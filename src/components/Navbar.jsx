  ...
  return (
    <nav
      className={`fixed w-full z-50 glass-nav transition-all duration-300 ${
        shrunk ? 'shadow-md bg-white/80 backdrop-blur-xl' : 'bg-white/70 backdrop-blur-lg'
      }`}
      aria-label="Primary"
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className={`flex justify-between items-center transition-all duration-300 ${shrunk ? 'h-16' : 'h-20'}`}>
          ...
          <div className="hidden md:flex items-center space-x-2">
            {['Home', 'About', 'Specialties', 'Chamber'].map((item, i) => (
              <a
                key={i}
      ...
      {/* Mobile Menu */}
      {open && (
        <div className="md:hidden glass-nav border-t border-white/40 w-full shadow-xl">
